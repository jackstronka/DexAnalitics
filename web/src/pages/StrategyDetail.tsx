...
export default function StrategyDetail() {
  ...
  const handleDelete = () => {
    if (
      ...
      !window.confirm(
        locale === 'pl'
          ? 'Usunąć tę strategię? Powiązane adresy pozycji zostaną usunięte na serwerze. Tej operacji nie da się cofnąć.'
  ...
  if (!strategy) {
    return <div className="text-center py-8">{locale === 'pl' ? 'Nie znaleziono strategii' : 'Strategy not found'}</div>
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-4">
        ...
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          ...
          {strategy.running ? (locale === 'pl' ? 'Działa' : 'Running') : (locale === 'pl' ? 'Zatrzymana' : 'Stopped')}
        </span>
        <div className="ml-auto flex flex-wrap gap-2">
          ...
          >
            ...
            {deleteMutation.isPending ? (locale === 'pl' ? 'Usuwanie...' : 'Deleting...') : (locale === 'pl' ? 'Usuń' : 'Delete')}
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>{locale === 'pl' ? 'Konfiguracja' : 'Configuration'}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-between gap-4">
              <span className="text-muted-foreground shrink-0">{locale === 'pl' ? 'Pula' : 'Pool'}</span>
              <span className="font-mono text-sm text-right">
                {strategy.pool_address
                  ? shortenAddress(strategy.pool_address, 6)
                  : locale === 'pl'
                    ? 'Per pozycja (Open Position)'
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">{locale === 'pl' ? 'Zaktualizowana' : 'Updated'}</span>
              <span>{formatDate(strategy.updated_at)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">{locale === 'pl' ? 'Dry run' : 'Dry run'}</span>
              <span>{strategy.dry_run ? (locale === 'pl' ? 'Tak' : 'Yes') : (locale === 'pl' ? 'Nie' : 'No')}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">{locale === 'pl' ? 'Auto-execute' : 'Auto-execute'}</span>
              <span>{strategy.auto_execute ? (locale === 'pl' ? 'Tak' : 'Yes') : (locale === 'pl' ? 'Nie' : 'No')}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{locale === 'pl' ? 'Parametry' : 'Parameters'}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {strategy.parameters.rebalance_threshold_pct !== undefined && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">{locale === 'pl' ? 'Próg rebalance' : 'Rebalance Threshold'}</span>
                <span>{strategy.parameters.rebalance_threshold_pct}%</span>
              </div>
            )}
            {strategy.parameters.max_il_pct !== undefined && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">{locale === 'pl' ? 'Max IL' : 'Max IL'}</span>
            ...
            {(strategy.parameters.min_rebalance_interval_minutes !== undefined ||
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">{locale === 'pl' ? 'Min. interwał rebalance' : 'Min Rebalance Interval'}</span>
                <span>
            ...
            {strategy.parameters.range_width_pct !== undefined && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">{locale === 'pl' ? 'Szerokość zakresu' : 'Range Width'}</span>
                <span>{strategy.parameters.range_width_pct}%</span>
              </div>
            )}
            {strategy.parameters.retouch_offset_pct !== undefined && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">{locale === 'pl' ? 'Retouch offset' : 'Retouch Offset'}</span>
                <span>{strategy.parameters.retouch_offset_pct}%</span>
              </div>
            )}
            {strategy.parameters.candle_seconds !== undefined && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">{locale === 'pl' ? 'Sekundy świecy' : 'Candle seconds'}</span>
            ...
            {strategy.parameters.position_addresses &&
              strategy.parameters.position_addresses.length > 0 && (
                <div className="pt-2 border-t border-border">
                  <p className="text-xs text-muted-foreground mb-2">{locale === 'pl' ? 'Podpięte pozycje' : 'Linked positions'}</p>
      ...
      {strategy.description && (
        <Card>
          <CardHeader>
            <CardTitle>{locale === 'pl' ? 'Opis' : 'Description'}</CardTitle>
          </CardHeader>
          <CardContent>
