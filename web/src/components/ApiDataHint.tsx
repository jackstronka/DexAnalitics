...
export default function ApiDataHint() {
  ...
  const title = useMemo(
    () => L('Skąd biorą się dane w tym panelu?', 'Where does data in this panel come from?'),
    [locale],
  )
  return (
    <div className="rounded-lg border border-amber-500/40 bg-amber-500/5 px-4 py-2 text-sm text-muted-foreground">
      <button
        type="button"
      ...
      >
        ...
        <span className="text-xs text-muted-foreground">{open ? L('Ukryj', 'Hide') : L('Pokaż', 'Show')}</span>
      </button>
      {!open ? null : (
