...
**Live executor (ważne):** domyślnie (po staremu) range-exit **odpala natychmiast** close+open (`rebalance_on_range_exit_immediately=true`).  
Jeśli chcesz tryb “tylko sygnał OOR + rebalance dopiero po interwale”, ustaw `rebalance_on_range_exit_immediately=false`.
...
- **Live executor (ważne):** domyślnie (po staremu) `Periodic` odpala rebalance **niezależnie od tego, czy jesteś w zakresie** (`periodic_requires_out_of_range=false`).  
  Jeśli chcesz wariant “co N godzin, ale tylko gdy OOR”, ustaw `periodic_requires_out_of_range=true`.
