...
| `periodic` | Rebalans co **`periodic_interval_hours`** (domyślnie “po staremu”: niezależnie od in-range) | Przewidywalny rytm; **koszt tx** może być wyższy przy krótkim interwale. Ustaw `periodic_requires_out_of_range=true`, jeśli chcesz wariant “tylko gdy OOR”. |
