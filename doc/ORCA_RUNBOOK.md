...
- **Kapitał sesji rebalance (SESSION GL + opcjonalny cap bota):** runbook operatora — [`WALLET_GL.md` §6](WALLET_GL.md#6-rollout-operatora--session-gl--reopen-5a). Krótko: po close sprawdź `GET /wallets/session-balances` / panel na pozycji; przed włączeniem bota na sesję ustaw `CLMM_REOPEN_USE_SESSION_CAPITAL=1` tylko po reconcile GL↔lifecycle. **`T` (USD reopen)** nadal z lifecycle §6.1, nie z GL.
